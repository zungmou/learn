...
这是一个结合了 **Jekyll 静态博客** 与 **FastAPI 后端管理系统** 的个人网站项目。支持传统的长篇博文和类似微博/推特的短篇“动态”。

## 🚀 功能特性

- **双频道内容**:
    - `_posts`: 适合深度的长篇技术或生活博文。
    - `_moments`: 类似推文的短动态，首页直接显示全文，支持碎碎念。
- **混合时间线**: 首页自动按时间顺序混合排列文章和动态。
- **FastAPI 管理后端**:
    - 自动化日期处理：发布时无需手动输入日期 and 格式化文件名。
